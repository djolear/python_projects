...
df_areas = pd.read_csv('la.area.txt', sep = '\t')
...
def get_BLS_county_data(BLS_data_path, df_areas):
    ...
    df_bls_county = pd.read_csv(BLS_data_path, dtype=col_types, low_memory = False, sep = '\t')

    df_bls_county['series_id'] = df_bls_county['series_id                     ']
    df_bls_county['value'] = df_bls_county['       value']
    df_bls_county = df_bls_county.drop(["series_id                     ", "       value"], axis=1)
...
df_unemp_county[['FIPS', 'state', 'countyname', 'year', 'month',
                 'Employment', 'Labor_Force', 'Unemployment', 'Unemployment_Rate']].to_csv('BLS_county_employment.csv',
                                                                                           index=False)
